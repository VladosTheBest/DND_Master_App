...
const safeErrorMessages = new Map<string, string>([
  ...
  ["generated_entity_too_short", "The generated location or quest is below the documented minimum content length. Expand the GM content and every required player card, then retry only this proposal."],
  ["generated_entity_incomplete", "The generated location or quest is missing required player-facing cards. Add every card documented by propose_entity_create, then retry only this proposal."],
...
export function createDndMcpServer(client: DndMasterClient): McpServer {
  ...
  server.registerTool(
    "propose_entity_create",
    {
      ...
      description:
        "Create one persistent proposal for exactly one new entity or event. Input shape: { campaignId, prompt, kind, candidate, mediaIntents?, warnings? }. kind is location, player, npc, monster, quest, lore, or event; mediaIntents is allowed only for non-event entities. campaignId, prompt, and kind are top-level; candidate is one complete entity and must not contain kind, tempKey, id, or revision. Minimal quest example: {\"campaignId\":\"campaign-id\",\"prompt\":\"Create one quest\",\"kind\":\"quest\",\"candidate\":{\"title\":\"Quest title\",\"summary\":\"Short summary\",\"content\":\"Full quest details\"}}. A new location must be table-ready: candidate.content needs at least 1800 characters of GM-only detail, plus at least four playerCards of at least 1200 characters each titled 'Описание локации', 'Кого здесь можно встретить', 'Что можно найти', and 'Проверки и результаты'. The first is vivid multi-paragraph read-aloud narration; the others cover every established NPC present and interactions, searchable features/clues, and several concrete checks/DCs with tiered failure/success/exceptional results. Keep playerCards free of hidden truth and use meaningful detail rather than padding. A new quest needs at least 1800 characters of GM content and normally 2-4 playerCards of at least 1000 characters each; when combat is plausible, include 1-3 preparedCombats referencing only entity ids verified from the campaign and omit combat instead of inventing ids. Put free-form narrative in candidate.content and use simple candidate.title, candidate.summary, and candidate.tags where useful; use nested fields only when their disclosed schemas are known. Do not invent description, objectives, rewards, secrets, or roleplayCues fields. Never send top-level title, entities, entity, data, payload, quest, or entityKind. For a quest plus NPCs plus locations, call this tool separately for each item. This never inserts campaign data directly.",
  ...
  server.registerTool(
    "propose_entity_update",
    {
      ...
      description:
        "Create one persistent update proposal for exactly one existing entity or event. Call get_entity first. Input shape: { campaignId, prompt, kind, entityId, patch, candidate?, mediaIntents?, warnings? }; mediaIntents is allowed only for non-event entities. campaignId, prompt, kind, entityId, and patch are top-level, and patch contains only supported fields intentionally changed. For a media-only replacement of the entity's primary image, use patch: {} without candidate or placeholder mediaIntents, then call stage_proposal_media exactly once with this proposal id, field: art.url, and complete prompt, alt, and caption metadata. The staged image is selected by default; keep exactly one selected art.url image in that proposal. This leaves every non-media entity field unchanged and does not replace the current campaign image until the user applies the proposal. The server preserves omitted media, relationships, cards, dialogue, and combat data. Never send an entity bundle or put kind/entityId inside patch. This never updates campaign data directly.",
