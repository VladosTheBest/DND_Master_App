...
Краткий вход для AI-агента. Сверено с текущим рабочим кодом 14 сентября 2026 года, включая модуль персонажей. Это карта реализации, а не план разработки; перед изменением конкретного участка открой указанные исходники. Правила сопровождения: [AGENTS.md](AGENTS.md).
...
**Журнал сессий.** [SessionsPage](apps/web/src/features/sessions/SessionsPage.tsx) добавляет пункт «Сессии» выбранной кампании: нумерованный журнал, обзор, вкладки игроков, активность и полный текст с поиском/подсветкой и страницами по 50 реплик. Импорт UTF-8 TXT из Quill до 4 МиБ, название и защита от дублей; аудио/ZIP не принимаются. [session_transcripts.go](apps/server/internal/httpapi/session_transcripts.go) хранит `importedSessions` отдельно от campaignData в существующем store: лимит текстов 50 МиБ/кампанию, атомарное сохранение/откат, owner-доступ, удаление только импортированной копии. [session-stats.ts](apps/web/src/features/sessions/session-stats.ts) считает слова, реплики и объединённые интервалы речи каждого человека; выбор мастера исключает его из процентов и хранится локально в браузере. Доля речи не считается мерой интереса/вовлечённости; молчавшие участники из TXT не определяются.

**AI-анализ сессии.** [session_analysis.go](apps/server/internal/httpapi/session_analysis.go) сохраняет отдельный отчёт: summary, события, действия/моменты игроков, зацепки, спорные места и proposal IDs. `POST /api/ai/codex/prompts` принимает `sessionId`: проверка владельца, новый runId/контекст без изображений, результат подтверждается по сохранённому отчёту именно этого запуска с совпадающим digest. MCP читает весь текст страницами по 24 000 Unicode code points через `get_session_transcript` и сохраняет отчёт `save_session_analysis`; изменения сущностей остаются проверяемыми предложениями. Тайм-аут — три обычных (12 минут по умолчанию), HTTP write deadline продлевается; внешние прокси могут ограничивать время. Старый текст/анализ не стирается при сбое. Проверки: полный Go suite, 30 MCP-тестов, web build, [тест статистики](scripts/test-session-stats.mjs), изолированный браузерный импорт/поиск/игроки/активность/перезагрузка с синтетическим отчётом. Реальный AI-анализ пользовательского TXT и шестичасовой игры ещё не проверен. [Описание и API](docs/session-journal.md).
...
**Discord / Quill.** Внешнее локальное приложение самостоятельно подключается к Discord, требует согласия всех участников и сохраняет отдельные голоса/общий текст. Сайт не управляет ботом и не получает аудио/токен: пользователь импортирует TXT в свою кампанию. Одна активная сессия на одном сервере/канале поддерживается самим Quill. История, команды и ограничения описаны в [README Quill](../Quill/README.md); контракт импорта/анализа сайта — [журнал сессий](docs/session-journal.md).
...
| PUT | `/api/campaigns/{c}/sessions/{id}/analysis` | owner | отчёт с проверкой digest/принадлежности proposal IDs → [session_analysis.go](apps/server/internal/httpapi/session_analysis.go) |
