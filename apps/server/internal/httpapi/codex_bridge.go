...
2. Make new quest and location candidates table-ready, not skeletal: include 2-4 distinct playerCards for revealable scenes, clues, speeches, or handouts. When combat is plausible, include 1-3 preparedCombats using valid existing campaign entity ids (or valid blueprint temp keys inside a new-campaign proposal); never invent unresolved ids. Other narrative entities should also include useful playerCards when they have information players can discover.
...
func buildCodexProposalPrompt(input codexPromptInput) string {
	...
	builder.WriteString("For each newly created quest or location, include 2-4 useful playerCards with distinct titles and player-safe content. If a combat encounter is narratively plausible, include 1-3 preparedCombats referencing only real existing campaign entity ids; omit combat rather than inventing an id. For other narrative entities, add playerCards whenever they contain revealable information. ")
