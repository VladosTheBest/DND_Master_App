...
2. Make new quest and location candidates table-ready, not skeletal. Every new location must have at least 1800 characters of substantial GM-only content plus at least four long, distinct playerCards of at least 1200 characters each: (a) "Описание локации" with vivid read-aloud arrival text covering atmosphere, visible activity and present people without revealing secrets; (b) "Кого здесь можно встретить" listing every established NPC present, what players notice and how they can interact, using names rather than invented permanent ids; (c) "Что можно найти" covering visible features, searchable places, clues and useful discoveries; and (d) "Проверки и результаты" with several concrete D&D checks/DCs and tiered success, failure and exceptional-success outcomes. Each card must contain several meaningful paragraphs or a substantial structured list, never padding or a one-line placeholder. New quests must have at least 1800 characters of GM content and 2-4 similarly useful playerCards of at least 1000 characters each. When combat is plausible, include 1-3 preparedCombats using valid existing campaign entity ids (or valid blueprint temp keys inside a new-campaign proposal); never invent unresolved ids. Other narrative entities should also include useful playerCards when they have information players can discover.
3. Create every requested text proposal and collect its returned proposal id before generating or staging any image. A prose-only answer is a failure for this integration.
4. Finish only after each checklist item has a successful persistent propose_* result. Name every returned proposal id so the user can review it in the website's AI drafts inbox.
...
func buildCodexProposalPrompt(input codexPromptInput) string {
	...
	if input.IncludeImages {
		builder.WriteString("The user opted in to image generation for every created entity. Persistence order is mandatory: create every requested text proposal and collect every proposal id; only then use the built-in $imagegen skill once for each new non-event entity proposal and stage that entity's output as gallery media with complete title, prompt, alt, and caption metadata against its own proposal id. Do not skip an entity merely to save time. Never attach media to an event proposal; associate event scene art with the most relevant quest or location proposal. If generation or staging fails, keep all text proposals, do not duplicate them, and identify exactly which entity lacks an image in the final answer. ")
	} else {
	...
	builder.WriteString("Every newly created location must include at least 1800 characters of private GM content and at least four playerCards of at least 1200 characters each, titled and scoped as follows: 'Описание локации' is vivid multi-paragraph read-aloud arrival narration; 'Кого здесь можно встретить' covers all established NPCs present, visible traits and interactions; 'Что можно найти' covers features, searchable places, clues and discoveries; 'Проверки и результаты' gives several concrete D&D checks with DCs and tiered failure, success and exceptional-success results. Keep all playerCards free of hidden GM truth and fill them with useful details rather than padding. New quests need at least 1800 characters of GM content and 2-4 similarly substantial playerCards of at least 1000 characters each. If a combat encounter is narratively plausible, include 1-3 preparedCombats referencing only real existing campaign entity ids; omit combat rather than inventing an id. For other narrative entities, add playerCards whenever they contain revealable information. ")
